...
function stackingShapesEnsureAudio() {
  if (!window.stackingShapes.soundEnabled) return;
  const AC = window.AudioContext || window.webkitAudioContext;
  if (AC && !window.stackingShapes.audioCtx) window.stackingShapes.audioCtx = new AC();
  if (
...
function stackingShapesPlayPop(impact = 1) {
  if (!window.stackingShapes.soundEnabled || !window.stackingShapes.audioCtx) return;
...
function initSoundSystem() {
  ...
  if (soundToggle) {
    soundToggle.checked = !!window.stackingShapes.soundEnabled;
    soundToggle.addEventListener("change", async () => {
      window.stackingShapes.soundEnabled = soundToggle.checked;
      localStorage.setItem(
        "heroSound",
        JSON.stringify(window.stackingShapes.soundEnabled)
      );
      if (window.stackingShapes.soundEnabled) stackingShapesEnsureAudio();
      else if (
        window.stackingShapes.audioCtx &&
        window.stackingShapes.audioCtx.state === "running"
      ) {
        try {
          await window.stackingShapes.audioCtx.suspend();
        } catch (e) {}
      }
    });
  }
  ...
  if (voiceControlEl) {
    const syncVoiceLabel = () => {
      voiceControlEl.textContent = window.stackingShapes.soundEnabled
        ? " : On"
        : " : Off";
      voiceControlEl.setAttribute(
        "aria-pressed",
        String(!!window.stackingShapes.soundEnabled)
      );
    };
