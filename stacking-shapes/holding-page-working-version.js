// TEST COMMENT - This confirms file updates are working
/**
...
console.log("Script - new Holding Page v1.8.7");
...
function initStackingShapes() {
  ...
  console.log("Initial scale for shape creation:", initialScale);
  
  // Initialize shapes with responsive positioning and scaling
  window.stackingShapes.shapes = [
    bodyFromPath("shape1", innerWidth * 0.25, innerHeight * 0.25, "#094C45", "", "#news", initialScale),
    bodyFromPath("shape2", innerWidth * 0.5, innerHeight * 0.3, "#F7A3BC", "", "#founders", initialScale),
    bodyFromPath("shape3", innerWidth * 0.75, innerHeight * 0.3, "#FFB533", "", "#work", initialScale),
    bodyFromPath("shape4", innerWidth * 0.15, innerHeight * 0.15, "#88D3CD", "", "#about", initialScale),
  ];
  ...
  // Add debug logging for all shapes
  console.log("=== SHAPE SIZE DEBUG ===");
  console.log("Window Size:", { width: window.innerWidth, height: window.innerHeight });
  console.log("Shapes Count:", window.stackingShapes.shapes.length);
  
  window.stackingShapes.shapes.forEach((shape, index) => {
    ...
    console.log(`Shape ${index}:`, {
      ...
      fillColor: shape.render.fillStyle
    });
  });
  console.log("========================");
...
function addLogoSprite() {
  ...
  console.log("Logo creation scale:", currentScale);
  ...
  const logo = Matter.Bodies.fromVertices(
    innerWidth * 0.5,   // x position (responsive - center of canvas)
    innerHeight * 0.15, // y position (responsive - adjust this value to shift up/down)
    scaledLogoVertices,
    {
  ...
  // Add debug logging for logo scaling
  if (logo && logo.bounds) {
    const bounds = logo.bounds;
    const width = bounds.max.x - bounds.min.x;
    const height = bounds.max.y - bounds.min.y;
    console.log(`Logo created with scale ${currentScale}:`, {
      dimensions: { width: width, height: height },
      scale: currentScale,
      originalVertices: logoVertices.length,
      scaledVertices: scaledLogoVertices.length
    });
  }
...
function bodyFromPath(pathId, x, y, fill, label, link, scale = 1) {
  ...
  // Add debug logging for scaled shape
  if (b && b.bounds) {
    const bounds = b.bounds;
    const width = bounds.max.x - bounds.min.x;
    const height = bounds.max.y - bounds.min.y;
    console.log(`${pathId} created with scale ${scale}:`, {
      dimensions: { width: width, height: height },
      scale: scale,
      originalVertices: verts.length,
      scaledVertices: scaledVerts.length
    });
  }
...
function getResponsiveScale() {
  ...
  console.log("Responsive Scale Calculation:", {
    ...
    baseSize: { width: BASE_W, height: BASE_H },
    ratio: s,
    finalScale: scale
  });
  return scale;
...
function applyResponsiveScale() {
  ...
  console.log("Applying Responsive Scale:", {
    newScale: s,
    currentScale: window.stackingShapes.currentScale,
    ratio: ratio
  });
  
  if (Math.abs(ratio - 1) > 0.001) {
    window.stackingShapes.shapes.forEach((b, index) => {
      console.log(`Scaling shape ${index} by ratio:`, ratio);
      ...
      // Log new dimensions after scaling
      const bounds = b.bounds;
      const width = bounds.max.x - bounds.min.x;
      const height = bounds.max.y - bounds.min.y;
      console.log(`Shape ${index} after scaling:`, {
        dimensions: { width: width, height: height },
        position: { x: x, y: y }
      });
    });
